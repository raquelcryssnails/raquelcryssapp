...
export function ClientAuthProvider({ children }: { children: ReactNode }) {
  ...
  const loginClient = useCallback(async (email?: string, password?: string): Promise<{ success: boolean, error?: string }> => {
    ...
    try {
      ...
      // Check for specific, expected authentication errors
      if (error.code === 'auth/user-not-found' || 
          error.code === 'auth/wrong-password' || 
          error.code === 'auth/invalid-credential') {
        errorMessage = "E-mail ou senha inválidos.";
        // Log as a warning since this is an expected user error, not a system crash
        console.warn(`[ClientAuthContext] Failed login attempt for ${email}. Reason: ${errorMessage}`);
      } else {
        // Log other, unexpected errors as critical errors
        console.error("[ClientAuthContext] An unexpected error occurred during client login:", error);
