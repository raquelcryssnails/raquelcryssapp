  ...
  const login = useCallback(async (email?: string, password?: string): Promise<{success: boolean, error?: string}> => {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found' || 
        ...
        // Log as a warning for expected user errors
        console.warn(`[AuthContext] Failed admin login attempt for ${email}. Reason: ${errorMessage}`);
      } else {
        // Log unexpected errors
        console.error("[AuthContext] An unexpected error occurred during admin login:", error);
